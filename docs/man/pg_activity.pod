...
=item B<--rds>
	...
	Enable support for AWS RDS (implies --no-tempfiles).
...
the same as thoses described when data_directory is not readable.
...
function to fetch the information.  When the number of tempfile grows a lot,
the query might also timeout. The first failure to access this data will Temp
file stats can be completely skipped with B<--no-tempfile>.
...
Finally, some information are not available in older version of PostgreSQL,
